...
class ReceiveResponseEvent(WSManEvent):
    ...
    def exit_code(self) -> t.Optional[int]:
        """On completion this is the exit code of the command."""
        rc = self._raw.find("s:Body/rsp:ReceiveResponse/rsp:CommandState/rsp:ExitCode", namespaces=NAMESPACES)
        return int(rc.text or "-1") if rc is not None else None
